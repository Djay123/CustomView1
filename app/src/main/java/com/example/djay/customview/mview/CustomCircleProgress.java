...
public class CustomCircleProgress extends View{
    ...
    public static final int DEFAULTMAXVALUE= 10000;
    //圆的宽度
    ...
    public CustomCircleProgress(Context context, AttributeSet attrs) {
        ...
        startAngles = a.getInteger(R.styleable.CustomCircleProgress_startAngles,
                DEFAULTEND);
        endAngles = a.getInteger(R.styleable.CustomCircleProgress_endAngles,
                DEFAULTEND);
        widthCircle = (int) a.getDimension(R.styleable.CustomCircleProgress_circleWidth,
                DEFAULTWIDTH);
        radius = a.getDimension(R.styleable.CustomCircleProgress_radius,
                100);
        mMaxValue = a.getInteger(R.styleable.CustomCircleProgress_maxValue,
                DEFAULTMAXVALUE);
        a.recycle();
    ...
    private void drawText(Canvas canvas) {
        if(hitPaint != null){
            canvas.drawText("今天走了",centerPoint.x,offsetHit,hitPaint);
        }
        if(valuePaint != null){
            canvas.drawText((int)(value) + "",centerPoint.x,offsetValue,valuePaint);
        }
        if (unitPaint != null){
            canvas.drawText("步",centerPoint.x,offsetUnit,unitPaint);
        }
    }
